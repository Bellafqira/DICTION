...
class GaussianMixture(torch.nn.Module):
    ...
    def _init_params(self):
        if self.mu_init is not None:
            assert self.mu_init.size() == (1, self.n_components,
                                           self.n_features), "Input mu_init does not have required tensor dimensions " \
                                                             "(1, %i, %i)" % (
        ...
        if self.var_init is not None:
            assert self.var_init.size() == (1, self.n_components,
                                            self.n_features), "Input var_init does not have required tensor " \
                                                              "dimensions (1, %i, %i)" % (
                self.n_components, self.n_features)
            # (1, k, d)
    ...
    def predict(self, x, probs=False):
        ...
        """
        x = self.check_size(x).cuda()

        weighted_log_prob = self._estimate_log_prob(x).cuda() + torch.log(self.pi).cuda()
    ...
    def _estimate_log_prob(self, x):
        ...
        mu = self.mu.cuda()
        prec = torch.rsqrt(self.var.cuda()).cuda()
